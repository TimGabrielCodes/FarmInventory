...
public class ManagerController extends HttpServlet {
    ...
    public void managerDashBoard(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        try {
           ...
           // List<Product> list = productDAO.get();
            ...
            dispatcher = request.getRequestDispatcher("/Views/Manager/Dashboard.jsp");
            dispatcher.forward(request, response);
        } catch (IOException ex) {
            Logger.getLogger(StockController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    public void transactions(HttpServletRequest request, HttpServletResponse response) throws ServletException {
        try {
            List<Transaction> list = productDAO.getTrans();

            request.setAttribute("list", list);
            request.setAttribute("title", "Inventory Products List");
            dispatcher = request.getRequestDispatcher("/Views/Manager/ViewTransactions.jsp");
            dispatcher.forward(request, response);
        } catch (IOException ex) {
            Logger.getLogger(StockController.class.getName()).log(Level.SEVERE, null, ex);
        }
   ...
   public void addProducts(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        dispatcher = request.getRequestDispatcher("/Views/Manager/addProduct.jsp");
        dispatcher.forward(request, response);
   ...
   private void makeTransaction(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
         ...
         request.setAttribute("title", "Make Transaction");
          dispatcher = request.getRequestDispatcher("/Views/Manager/MakeTransaction.jsp");
           dispatcher.forward(request, response);
 ...
 List<Transaction> list = productDAO.getUserTransactions(user.getFullName());
    ...
    private void anayltics(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
       dispatcher = request.getRequestDispatcher("/Views/Manager/Analytics.jsp");
     request.setAttribute("title", "Inventory Analysis");
